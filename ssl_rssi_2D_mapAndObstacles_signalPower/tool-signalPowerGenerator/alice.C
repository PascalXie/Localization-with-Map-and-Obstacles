...
int main()
{
	...
	//
	ofstream write3("Data_PathLossExponent.txt");
	write3<<"Power_node "<<Power_node_ref<<endl;
	write3<<"Power_ref "<<Power_ref<<endl;
	write3<<"PathLossExponent "<<gamma<<endl;
	write3<<"d0 "<<1<<endl;
	write3.close();
	...
	double nx = 56.8591;
	double ny = -67.0267;
	double d = sqrt((ax-nx)*(ax-nx)+(ay-ny)*(ay-ny));
	double S_a = Power_ref -  10.*gamma*log10(d/1.);
	cout<<"S_a "<<S_a<<endl;
...
void generateComparingData(ToolSignalPowerGenerator * spg, double pathLossExponent)
{
	...
	for(int i=0;i<binSize;i++)
	{
		...
		// path loss exponent
		double d_0 = 1;
		double Strength_PLE = Strength_ref - 10.*ple*log10(distance/d_0);
